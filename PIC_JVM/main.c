...
#define NOP 0x00
#define IADD 0x60
unsigned char java_program[] = { 0x32, 0x53, 0x60, BREAKPOINT,NOP };
...
int main(void)
{
    ...
    init_byte_code();
    printf("Java VM 0.00005\n");

    while ( java_program[pc] != 0xca )
    {
        op=java_program[pc];
        print_bytecode(java_program,pc);
        printf("\n");
        pc = execute_bytecode(java_program,pc);
        printf(" (%d)\n",result);
    }
...
/*
    1) look-up byte code,
    2) number of parameters and call a function
...
multianewarray c5 1100 0101 3: indexbyte1, indexbyte2, dimensions count1, [count2,...] → arrayref create a new array of dimensions dimensions with elements of type identified by class reference in constant pool index (indexbyte1 << 8 + indexbyte2); the sizes of each dimension is identified by count1, [count2, etc.]
